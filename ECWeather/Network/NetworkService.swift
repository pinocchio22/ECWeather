...
class NetworkService {
    static func getCurrentWeather(lat: Double, lon: Double, completion: @escaping (CustomWeather?) -> Void) {
        let apiUrl = "https://api.openweathermap.org/data/2.5/weather?lat=\(lat)&lon=\(lon)&appid=\(Key.apiKey.rawValue)&\(DataManager.shared.temperatureType1.rawValue)"
        // API 요청 및 디코딩
    ...
    static func getCurrentWeather(cityName: String, completion: @escaping (CustomWeather?) -> Void) {
        let apiUrl = "https://api.openweathermap.org/data/2.5/weather?q=\(cityName)&appid=\(Key.apiKey.rawValue)&\(DataManager.shared.temperatureType1.rawValue)"
        // API 요청 및 디코딩
        AF.request(apiUrl).responseDecodable(of: CurrentWeather.self) { response in
            switch response.result {
            case .success(let currentWeather):
    ...
    static func getWeeklyWeather(cityName: String, completion: @escaping ([CustomWeeklyWeather]?) -> Void) {
        let apiUrl = "https://api.openweathermap.org/data/2.5/forecast?q=\(cityName)&appid=\(Key.apiKey.rawValue)&\(DataManager.shared.temperatureType1.rawValue)"
    ...
    static func getWeeklyWeather(lat: Double, lon: Double, completion: @escaping ([CustomWeeklyWeather]?) -> Void) {
        let apiUrl = "https://api.openweathermap.org/data/2.5/forecast?lat=\(lat)&lon=\(lon)&appid=\(Key.apiKey.rawValue)&\(DataManager.shared.temperatureType1.rawValue)"
    ...
    static func getIcon(iconCode: String) -> Data {
