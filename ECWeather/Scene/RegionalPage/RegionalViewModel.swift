...
//    ]
    ...
    func getCustomAnnotation(cityName: String) -> CustomAnnotation {
        var locationList: CustomAnnotation?
        NetworkService.getCurrentWeather(cityName: cityName) { item in
            if let item = item {
                locationList = CustomAnnotation(
                    title: cityName,
                    subtitle: String(item.currentTemp),
                    iconImage: UIImage(data: NetworkService.getIcon(iconCode: item.icon)),
                    coordinate: CLLocationCoordinate2D(latitude: item.lat, longitude: item.lon)
                )
            }
        }
        return locationList ?? CustomAnnotation(coordinate: CLLocationCoordinate2D(latitude: 37.514575, longitude: 127.0495556))
    }
